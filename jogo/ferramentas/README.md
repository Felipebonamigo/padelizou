...
stdout se repete e que semente fora da faixa é recusada.
...
`bloq.` a taxa de golpes bloqueados, `pontos` na mediana, `final.` a média de finalizações e a
onda que mais tirou vidas.
...
soco não interrompe). Sem inimigo na tela com a tela travada, vão pro meio dela — é o que faz
quem ainda está lá fora entrar.
...
- Um jogador só (sem P2) e sem as melhorias do Templo, quando elas existirem.
- O bot não usa arremesso pra atropelar grupo de propósito, nem joga pelo cenário.
