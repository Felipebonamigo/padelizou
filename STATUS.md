...
> ⚠️⚠️ **O BACKUP PRA FORA DO SERVIDOR ESTÁ PARADO DESDE 04/08** — `rclone` perdeu a autorização do Google Drive e falha todo dia às 4h30 ("Google Drive ainda não autorizado"). O backup LOCAL roda normal (4h, 19 cópias guardadas em `/var/backups/padelizou`), mas **se o VPS morrer não há cópia em outro lugar**. Conserto é interativo e só o Felipe pode fazer: `rclone config reconnect padelizou-drive:` (pede login do Google no navegador).
