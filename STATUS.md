...
>
> 🌓 **Tema escuro tem DOIS caminhos, e a diferença é a razão de existirem os dois.** Logo **preto puro** (Paralelo) o CSS inverte pra branco — `LogoEscuro`. Logo **colorido** (Grand Padel, azul) recebe a **arte branca oficial da marca** — `ImagemEscura`, troca de ARQUIVO e não de filtro: `invert()` em azul dá laranja, e marca alheia não se recolore.
>
> 🛑 **O ERRO DO BLOCO, e ele quase foi publicado: EU REDESENHEI O LOGO DO GRAND PADEL.** A arte tinha chegado **colada na conversa** (pixels na mensagem, não arquivo em disco), e em vez de dizer isso eu recriei o logo à mão em vetor — "fiel o bastante" pros 26px do rodapé. O Felipe reparou na hora: *"você alterou a logo original, o patrocinador não vai gostar"*. Estava certo: **logo é identidade registrada, e o dono repara**. Saiu do repositório antes de qualquer deploy. Virou **regra escrita no topo de `Patrocinador`** e **teste-trava** (`So_entra_na_lista_quem_mandou_a_arte`): quem somar um nome na lista tem que mexer nessa linha e topar com a regra.
>
> 📦 **Imagem COLADA não é arquivo — anexo ou git, não tem terceira via.** A arte oficial só chegou quando o Felipe empurrou uma branch (`git push origin HEAD:arte-grand-padel`), e ela veio commitada **dentro do `wwwroot`** — onde os 4,4 MB de `.ai` e `.pdf` ficariam baixáveis por qualquer um e entrariam em todo pacote de deploy. **O kit agora mora em `arte/patrocinadores/grand-padel/`** (raiz do repositório, fora do projeto e portanto fora do `dotnet publish` — conferido no pacote gerado: nenhum arquivo do kit, só os três `.webp` de 27 KB). O `README.md` de lá registra qual variante vira qual arquivo e a receita de conversão. **A branch `arte-grand-padel` virou redundante** e pode ser apagada: `git push origin --delete arte-grand-padel`.
>
> 🌍 **A régua de exibição inverteu no mesmo dia.** Nasceu valendo **só no dev** (`Beta__AmbienteDeTeste`), pra nenhum logo aparecer em produção antes de contrato fechado. Fechado o contrato, a lista do código passou a valer **em todo ambiente** — quem paga pra aparecer não pode depender de alguém lembrar de editar o systemd da produção. **Configuração preenchida (`Patrocinadores__Lista__0__...`) substitui a lista inteira**, e é a saída de emergência: trocar um logo no ar, ou apagar a faixa, sem esperar deploy.
>
> 🔗 **Os logos são links** (`paraleloesporte.com.br`, `grandpadel.com.br`) com **`rel="sponsored"`** — o rótulo que o Google exige em link pago; sem ele patrocínio conta como compra de link e quem paga a multa de ranqueamento é o Padelizou. Como viraram alvo de toque, o padding levou a área clicável de **26px pra ~41px** (a régua de dedo é ~44) e o `gap` descontou o mesmo tanto, então o desenho não mudou. E os `<img>` ganharam **`asp-append-version`**: nome de arquivo se repete quando a marca manda arte nova, e sem o carimbo o visitante continuaria vendo a antiga do cache.
>
> 📱 **De quebra, o vão escuro depois do rodapé no iPhone.** Relato com print: *"o final ali ficou muito espaço em branco"*. A página termina exatamente no rodapé (medido) — o que aparecia era o **canvas ALÉM do documento**: o elástico do fim da rolagem pintava com o fundo do `body` (`#0f1526`), que no tema escuro não é a cor do rodapé (`#1a2338`). O `html` passou a levar `background-color: var(--pdz-surface)` e o quique virou o rodapé continuando. No tema claro ninguém nunca notou porque as duas cores são quase iguais.
>
> 🧪 **4.781 testes, 0 falhas (7 em `PatrocinadoresDoSiteTests`).** Guardam o que custa caro: que produção mostre a lista **sem depender de configuração**, que o systemd continue podendo substituí-la, que logo colorido troque de arquivo em vez de ser invertido, que patrocinador sem imagem não vire texto solto, que **todo patrocinador tenha link `https://`** — e a trava da arte oficial.
>
> ✅ **PUBLICADO em 21/08/2026, na `build-661-7b0bafc`, em dev E em prod.** **Sem migration**: nada de banco neste bloco.
>
> ✅ **A PENDÊNCIA DE INFRA DE 19-20/08 CAIU:** os secrets **`VPS_SSH_KEY` e `VPS_KNOWN_HOSTS` entraram no environment `dev`**. O deploy pro dev pelo workflow passou (run #36, step "Preparar o acesso ao VPS" em ~1s) — **dev voltou a ser publicável pelo GitHub e pelo celular**, sem SSH na mão.
>
> 🚨 **PENDÊNCIA NOVA, E É DE SEGURANÇA: O ENVIRONMENT `prod` NÃO TEM APROVAÇÃO OBRIGATÓRIA.** O `deploy.yml` comenta que "a regra mora no environment prod", mas ela **não está configurada**: o deploy de produção disparado hoje entrou em execução **6 segundos** depois de criado, sem passar por `waiting`. Na prática, qualquer disparo do workflow — de qualquer sessão — publica direto em produção. Conserto: **Settings → Environments → `prod` → Required reviewers**, adicionando o Felipe. ⚠️ **NÃO marcar "Prevent self-review"**: operador solo que se proíbe de aprovar a si mesmo deixa a produção intransitável. **Só dá pra fazer na interface** — a API de environments é bloqueada pelo proxy das sessões do Claude (`Access to this GitHub API path is not permitted through this proxy`).
>
> ⚠️ **NÃO CONFERIDO NO NAVEGADOR EM PRODUÇÃO** — a política de rede da sessão bloqueia `padelizou.com.br` com 403 no CONNECT (mesma nota de 19 e 20/08). O `deploy.sh` respondeu `==> Feito. build-661-7b0bafc no ar em prod` e o healthcheck passou, senão o próprio script teria dado rollback. **A prova barata, sem login:** abrir `/image/patrocinadores/grand-padel.webp` em produção — se responder a imagem, a faixa está no ar.
>
...
>
> 👻 **O BOTÃO ESTAVA LÁ E NINGUÉM CONSEGUIA VER — corrigido em 12/08.** A 1ª versão usou `btn-link text-muted`: um lápis de **13px em `#9aa7c4`**, sem borda, sem fundo, na última coluna, que **só acendia no `:hover`** (aí o `btn-link` vira `#A3D827`, o verde da marca). A descrição de quem usou foi literal: *"estou buscando no escuro com o mouse"*. Agora é botão **com palavra** — "Corrigir" e "Apagar", sob o cabeçalho **Ações** —, que é como o resto do app faz. ⚠️ **Borda não resolveria**: no tema escuro a casa desenha `btn-outline-*` com `--pdz-border`, que é **10% de opacidade** (site.css, "Botão de opção não escolhido"). Medido nos **dois** temas com o site rodando, não no olho.
>
> 🕳️ **E a ordem das linhas dançava**: a lista ordenava só por `DataJogo`, que guarda o **dia** — com 5 jogos na mesma noite o desempate ficava por conta do banco e a ordem mudava de um F5 pro outro. Fecha com o `Id`. É a mesma regra de sempre: **ordenação que decide o que a pessoa vê tem que ser TOTAL.**
>
> 🔐 **Quem pode mexer: quem administra a panelinha, QUEM LANÇOU o jogo e OS QUATRO QUE JOGARAM** (mais o admin do sistema como chave-mestra). ⚠️ A versão estreita — só admin + quem lançou — **foi pro ar e travou na hora**: no grupo real *Sub 90*, com 5 jogos, **2 eram intocáveis** pra quem estava olhando. Numa panelinha o jogo é lançado por quem pegou o celular primeiro, então o erro fica preso até essa pessoa aparecer. Quem estava na quadra sabe o placar, e o ranking é interno ao grupo. 🧪 **O teste negativo teve que trocar de personagem**: ele usava o `j[2]`, que **joga** a partida — com a regra nova continuaria verde provando o **contrário** do próprio nome. O de fora virou um quinto membro. A tela também parou de repetir a condição: `PodeMexerNoJogo` responde **uma vez** no controller e a view recebe o conjunto pronto. Duas armadilhas fechadas de propósito: **o grupo do jogo não se mexe** (trocar levaria os pontos pra outro ranking e o recálculo só passa em um deles) e **quem jogou mas não é mais membro entra na lista do formulário** — sem isso o `<select>` abriria sem a opção da pessoa, o navegador escolheria o primeiro nome, e corrigir UM PLACAR trocaria calado quem jogou.
>
> ✅ **Conferido no navegador com dado real, não só compilado**: correção invertendo o placar **virou o ranking geral** (3/3/1/1 → 1/1/3/3), apagar **zerou todo mundo sem sobra**, registrar de novo devolveu o 3/3/1/1. Os botões aparecem pra quem registrou **sem ser administrador**, pra quem **só jogou** a partida, e não aparecem pra membro que não é nada disso. Em produção: `cwd` do PID 3565654 em `build-524-6a6397a`, `NRestarts=0`, `/healthz` 200, **0 exceções**, `PontuacaoDaPanelinha`/`EditarJogo`/`ApagarJogo` dentro do `.dll` publicado.
>
