...
>
> 📧 **E o lembrete antigo mandava DOIS e-mails.** Ele chamava o funil de avisos (que já entrega caixa de entrada + e-mail + push num lugar só) **e** disparava e-mail por fora pelo `IEmailService` — sobra de quando o funil ainda não existia. O envio inline saiu; é o mesmo motivo pelo qual `DuplasController` não tem `IEmailService`. 🧪 **2.830 testes**, 26 novos, divididos de propósito: a **régua** (marcos, horário, texto) e a **varredura** inteira (achar os torneios certos, avisar os dois da dupla, gravar o marco, não repetir na hora seguinte, não cobrar o organizador de time). A varredura recebe o `agora` **por parâmetro** — é o que permite exercitá-la em dias que ainda não chegaram, em vez de conferir a régua e torcer pra ligação estar certa. ⚠️ **O que NÃO foi observado rodando**: o envio de ponta a ponta em ambiente real — são 22h e o serviço se cala fora da janela civilizada. A varredura foi exercitada inteira em teste, mas o primeiro disparo de verdade só dá pra confirmar amanhã de manhã no log.
>
