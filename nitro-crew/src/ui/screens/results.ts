...
export function resultsScreen(api: ScreenApi, data?: ScreenData): ScreenInstance {
  ...
  if (!d || !('results' in d)) {
    ...
    return { el: screenFrame('results', t('ui.results.title', { track: '?' }), back.el), nav: (nav) => listNav(list, nav, api.sfx) };
  }
  const { ctx } = api;
  ...
  const table = h('table', { class: 'table results-table' },
    h('thead', {}, h('tr', {},
      ...
      h('th', { text: t('ui.results.time') }), h('th', { text: t('ui.results.bestLap') }), h('th', { text: t('ui.results.points') }),
    )),
    h('tbody', {}, rows.map((r: RaceResultRow) => h('tr', { class: r.seat >= 0 ? 'human' : '', style: rowStyle(d.humans, r.seat) },
      h('td', { class: 'mono', text: String(r.position) }),
      h('td', { text: r.name }),
      h('td', { text: carName(r.carDefId) }),
      ...
      h('td', { class: 'mono', text: String(r.points) }),
    ))),
  );
  ...
  if (coop && d.humans.length > 0) {
    ...
    extras.push(h('p', { class: 'team-line', text: t('ui.results.team', { team: t('core.team.human'), points: teamRaceScore(d.results, teamId), rank: teamRaceRank(d.results, teamId) }) }));
  }
  ...
  const el = screenFrame('results', t('ui.results.title', { track: d.trackDef.name }),
    extras,
...
export function standingsScreen(api: ScreenApi, data?: ScreenData): ScreenInstance {
  ...
  if (!d || !('champ' in d) || !('cup' in d)) {
    ...
    const list = createFocusList([back], { sfx: api.sfx });
    return { el: screenFrame('standings', t('ui.standings.title', { cup: '?' }), back.el), nav: (nav) => listNav(list, nav, api.sfx) };
  }
  ...
  const driverTable = h('table', { class: 'table standings-table' },
    h('thead', {}, h('tr', {},
      h('th', { text: '#' }), h('th', { text: t('ui.results.name') }), h('th', { text: t('ui.standings.points') }), h('th', { text: t('ui.standings.wins') }),
      Array.from({ length: raceCount }, (_, i) => h('th', { class: 'mono', text: t('ui.standings.race', { n: i + 1 }) })),
    )),
    h('tbody', {}, top.map((s: StandingRow) => h('tr', { class: s.seat >= 0 ? 'human' : '', style: rowStyle(humans, s.seat) },
      h('td', { class: 'mono', text: String(champ.standings.indexOf(s) + 1) }),
      h('td', { text: s.name }),
      h('td', { class: 'mono', text: String(s.points) }),
      h('td', { class: 'mono', text: String(s.wins) }),
      s.positions.map((p) => h('td', { class: 'mono', text: p > 0 ? String(p) : '–' })),
    ))),
  );
  const teamTable = h('table', { class: 'table teams-table' },
    h('thead', {}, h('tr', {}, h('th', { text: '#' }), h('th', { text: t('ui.standings.teams') }), h('th', { text: t('ui.standings.points') }))),
    h('tbody', {}, champ.teams.map((tm, i) => h('tr', { class: tm.isHuman ? 'human' : '', style: tm.isHuman && humans.length > 0 ? `--seat:${humans[0].color}` : '' },
      h('td', { class: 'mono', text: String(i + 1) }),
      h('td', { text: tm.name }),
      h('td', { class: 'mono', text: String(tm.points) }),
    ))),
  );
  ...
  if (champ.completed) status = h('div', { class: 'verdict good big', text: t('ui.standings.champion') });
  else if (champ.eliminated) status = h('div', { class: 'verdict bad big', text: t('ui.standings.eliminated') });
  else status = h('p', { class: 'status-line', text: t('ui.standings.next', { n: champ.raceIndex + 1, m: raceCount, track: nextDef?.name ?? next ?? '?' }) });
  ...
  const list = createFocusList(items, { sfx: api.sfx });
  const el = screenFrame('standings', t('ui.standings.title', { cup: t(`core.cup.${cup.id}`) }),
    status,
    h('div', { class: 'standings-columns' },
      h('div', { class: 'table-wrap' }, h('h2', { class: 'sub-title', text: t('ui.standings.drivers') }), driverTable),
      h('div', { class: 'table-wrap' }, h('h2', { class: 'sub-title', text: t('ui.standings.teams') }), teamTable),
    ),
    h('div', { class: 'actions' }, items.map((i) => i.el)),
  );
  return { el, nav: (nav) => listNav(list, nav, api.sfx) };
