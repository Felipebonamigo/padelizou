...
## Modos
- **Campeonato** (co-op ou versus): 8 copas de 4 pistas por país, salvas a cada corrida. Solo/versus: seguir exige top 5 na corrida.
...
(mantém 35% para conseguir esterçar), nitro em reta livre, box pelo consumo medido volta a volta contra o que falta até o próximo box ou a chegada (folga de 1,15; `sim/fuel.ts`), elástico em relação ao
...
src/game      contracts.ts (interfaces), session.ts (laço), raceEnd.ts (fim de corrida: recordes, copa, carreira, estatísticas, conquistas),
              career-session/career-save (carreira), online-session.ts (lockstep na sessão), stats.ts, achievements.ts, rumble.ts,
              settings/save (localStorage) + cloudsave.ts (arquivo no Electron), errors.ts (relatório), desktop.ts (Electron)
src/net       protocol.ts (mensagens validadas), lockstep.ts (puro), client.ts (WebSocket)
server/       relay.mjs (salas por código, sucessão de anfitrião; dependência própria: ws)
src/i18n      registerStrings por namespace, t(), PT-BR/EN
desktop/      Electron + steamworks.js
scripts/      smoke.ts (corrida sem interface), balance.ts (IA×IA por pista), career-balance.ts (carreira), playtest*.mjs e
              pistas-ui.mjs (Chromium: geral, online, controles, recordes, telas de pista), diag.ts
tests/        vitest
```
