...
namespace padelizou.Controllers
{
    ...
    public class AgendaController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (jogador.AgendaMostrarMarcacoes)
            {
                var marcacoes = await _context.MarcacoesJogo
                    .Include(m => m.Clube)
                    .Include(m => m.QuadraClube)
                    .Where(m => m.JogadorId == userId && m.Status == "Confirmada")
                    .ToListAsync();

                itens.AddRange(marcacoes.Select(m => new AgendaItem
                {
                    Data = m.DataHora,
                    Tipo = "Marcação",
                    Titulo = $"{m.Clube.Nome} — {m.QuadraClube.Nome}",
                    Subtitulo = $"{m.DuracaoMinutos} min",
                    Icone = "bi-calendar2-plus",
                    LinkController = "MarcarJogo",
                    LinkAction = "MinhasMarcacoes"
                }));
            }
        ...
        public async Task<IActionResult> AtualizarPreferencias(
            bool mostrarJogosSemanais, bool mostrarTorneios, bool mostrarAulas, bool mostrarAlunos, bool mostrarMarcacoes)
        {
            var userId = int.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
            var jogador = await _context.Jogadores.FindAsync(userId);
            if (jogador != null)
            {
                ...
                jogador.AgendaMostrarMarcacoes = mostrarMarcacoes;
        ...
        public async Task<IActionResult> EventosJson()
        {
            ...
            if (jogador.AgendaMostrarMarcacoes)
            {
                ...
                eventos.AddRange(marcacoes.Select(m => (object)new
                {
                    title = $"{m.Clube.Nome} — {m.QuadraClube.Nome}",
                    start = m.DataHora,
                    end = m.DataHora.AddMinutes(m.DuracaoMinutos),
                    color = "#8e44ad",
                    url = Url.Action("MinhasMarcacoes", "MarcarJogo")
                }));
            }
        ...
        public async Task<IActionResult> Feed(int id, Guid token)
        {
            ...
            if (jogador.AgendaMostrarMarcacoes)
            {
                var marcacoes = await _context.MarcacoesJogo
                    .Include(m => m.Clube)
                    .Include(m => m.QuadraClube)
                    .Where(m => m.JogadorId == jogador.Id && m.Status == "Confirmada" && m.DataHora >= DateTime.Today)
                    .ToListAsync();

                eventos.AddRange(marcacoes.Select(m => new IcsEvent(
                    Uid: $"padelizou-marcacao-{m.Id}@padelizou.com.br",
                    Inicio: m.DataHora,
                    Fim: m.DataHora.AddMinutes(m.DuracaoMinutos),
                    DiaInteiro: false,
                    Resumo: $"Jogo marcado em {m.Clube.Nome}",
                    Local: $"{m.QuadraClube.Nome}, {m.Clube.Endereco}",
                    Descricao: null)));
            }
