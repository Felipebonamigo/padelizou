...
2. **Já existe algo equivalente aqui?** São 35 controllers (em 75 arquivos — muitos são classes parciais) e ~7.260 testes — reimplementar o
...
dotnet test Padelizou.slnx -c Release --no-build --nologo   # ~1min20, ~7.260 testes
...
sozinho. Quem mexer em JS com conferidor roda antes de commitar — hoje são **11**, e o glob
acima é a lista de verdade: `ls Padelizou.Tests/js/conferir-*.js`. (Esta linha já nomeou cinco
deles, um com o nome errado — `chave-fichas`, quando o arquivo é `fichas-da-chave`. Lista escrita
à mão num arquivo que ninguém roda envelhece calada; o glob não.)
...
- **Todo defeito corrigido vira teste de regressão** — é assim que a suíte foi de ~1000 pra
  ~7.260 testes, e é o que faz o STATUS.md confiável: se algo quebrou antes, tem teste hoje.
