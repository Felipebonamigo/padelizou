...
public class MuralDeParceirosTests
{
    ...
    public void So_inscricao_solo_de_torneio_aberto_recebe_chamado()
    {
        ...
        Assert.Null(MuralDeParceiros.MotivoParaNaoChamar(aberta, TorneioCom("Inscrições Abertas"), 2, NaoEstouInscrito, jaComecouAJogar: false));

        // A própria inscrição não se chama.
        Assert.NotNull(MuralDeParceiros.MotivoParaNaoChamar(aberta, TorneioCom("Inscrições Abertas"), 1, NaoEstouInscrito, jaComecouAJogar: false));

        // Dupla completa não procura ninguém.
        var completa = new Dupla { CategoriaId = 9, Jogador1Id = 1, Jogador2Id = 3 };
        Assert.NotNull(MuralDeParceiros.MotivoParaNaoChamar(completa, TorneioCom("Inscrições Abertas"), 2, NaoEstouInscrito, jaComecouAJogar: false));
        ...
        Assert.Null(MuralDeParceiros.MotivoParaNaoChamar(aberta, TorneioCom("Chaves em Sorteio"), 2, NaoEstouInscrito,
            jaComecouAJogar: false));

        // O que fecha o mural é a bola rolar pra AQUELA dupla.
        Assert.NotNull(MuralDeParceiros.MotivoParaNaoChamar(aberta, TorneioCom("Fase de Grupos"), 2, NaoEstouInscrito,
            jaComecouAJogar: true));

        // Linha de time não tem parceiro.
        var time = new Dupla { CategoriaId = 9, Jogador1Id = 1, NomeTime = "Os Fortes" };
        Assert.NotNull(MuralDeParceiros.MotivoParaNaoChamar(time, TorneioCom("Inscrições Abertas"), 2, NaoEstouInscrito, jaComecouAJogar: false));

        Assert.NotNull(MuralDeParceiros.MotivoParaNaoChamar(null, TorneioCom("Inscrições Abertas"), 2, NaoEstouInscrito, jaComecouAJogar: false));
    }
    ...
    public void Quem_ja_tem_dupla_FECHADA_nesta_categoria_nao_chama()
    {
        ...
        var motivo = MuralDeParceiros.MotivoParaNaoChamar(aberta, TorneioCom("Inscrições Abertas"), 2, euJaFechei, jaComecouAJogar: false);
    ...
    public void Quem_esta_inscrito_SOZINHO_na_mesma_categoria_continua_podendo_chamar()
    {
        ...
        Assert.Null(MuralDeParceiros.MotivoParaNaoChamar(aberta, TorneioCom("Inscrições Abertas"), 2, euEstouSozinho, jaComecouAJogar: false));
    ...
    public void Dupla_fechada_em_OUTRA_categoria_nao_impede_o_chamado()
    {
        ...
        Assert.Null(MuralDeParceiros.MotivoParaNaoChamar(aberta, TorneioCom("Inscrições Abertas"), 2, fecheiNoutraCategoria, jaComecouAJogar: false));
    }
