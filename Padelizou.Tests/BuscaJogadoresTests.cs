...
public class BuscaJogadoresTests
{
    ...
    [Fact]
    public async Task Perfil_privado_nao_muda_a_busca()
    {
        // ⚠️ TESTE-GUARDA, e ele nasceu de um erro de entendimento meu (10/08/2026): cheguei
        // a esconder cidade, categorias, clubes, pontos e time de quem tem perfil privado, e
        // a tirar essa pessoa dos filtros. O Felipe corrigiu a régua: **dado público aparece
        // pra todo mundo** — se foi campeão, os pontos do ranking, o clube onde joga. A chave
        // de perfil privado é sobre CONTATO (Instagram e WhatsApp), não sobre resultado de
        // padel, que já está no ranking, na chave do torneio e no histórico do parceiro.
        //
        // Este teste existe pra que a "melhoria de privacidade" não seja reintroduzida por
        // quem ler a tela e achar, como eu achei, que ela promete mais do que promete.
        using var ctx = TestInfra.NovoContexto();
        var time = new Time { Nome = "Nata Padel" };
        var clube = new Clube { Nome = "Arena", Contato = "x", Endereco = "y" };
        ctx.Times.Add(time);
        ...
        // O card vem inteiro, como o de qualquer um.
        var achado = (await Buscar(ctx, q: "Marina")).Resultados.Single();
        Assert.Equal("Nata Padel", achado.Time);
        Assert.Contains("2ª Masculina", achado.Categorias);
        Assert.Contains("Arena", achado.Clubes);

        // E ela continua achável pelos quatro filtros — sumir de um deles seria a mesma
        // trava de novo, só que pela porta de trás.
        Assert.Single((await Buscar(ctx, estado: "RS")).Resultados);
        Assert.Single((await Buscar(ctx, cidade: "Caxias do Sul")).Resultados);
        Assert.Single((await Buscar(ctx, categoriaId: categoria.Id)).Resultados);
        Assert.Single((await Buscar(ctx, clubeId: clube.Id)).Resultados);
    }
